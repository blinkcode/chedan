...
import {getActorByID} from "@/api/actor"
...
export default {
    ...
    methods: {
        commit() {
            ...
            if(!this.id){
                addActor(headerKey, introduction, name).then(res => {
                    if (res.code == 0) {
                ...
                editActor(id, headerKey, introduction, name).then(res => {
                    if (res.code == 0) {
                        this.$message.success("演员信息更新(添加)成功");
                        this.actor = res.result
                        this.form.name = this.actor.name
